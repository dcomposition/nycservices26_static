        ...
        >
          ...
          <Tooltip
            ...
            content={({ active, payload }) => {
              ...
              const item = payload[0].payload as AgencyStat;
              return (
                <div className="max-w-xs rounded-md border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm">
                  <p className="font-medium text-slate-900">{item.agencyName}</p>
                  <p className="mt-1 text-slate-500">{item.agency}</p>
          ...
          />
          <Bar dataKey="requests" fill="#1d4ed8" radius={[0, 2, 2, 0]} barSize={16} isAnimationActive={false}>
            <LabelList
              dataKey="requests"
              position="right"
              offset={8}
